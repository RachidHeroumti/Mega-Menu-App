var vm = new StoreinoApp({
  ...
  methods: {
    ...
    EditItem(item) {
      ...
      this.name = item.slug;
      this.saveSlug = item.slug;
      this.url = item.url;
      this.optionShow = item.optionShow;
      this.AddSimpleItem = true;
    },
    ToEditStyle(item) {
      this.ToAddItemStyle = true;
      this.saveSlug = item.slug;
    },
    EditItemStyle() {
      this.itemStyle = this.styleType;
      this.selectedMenu.forEach((menuItem, index) => {
        if (menuItem.slug === this.saveSlug) {
          this.selectedMenu[index].type = this.itemStyle;
        } else {
          
        }
      });
      console.log("the updated menu ", this.selectedMenu);

      this.ToAddItemStyle = false;
    },
    //-------------------------
    DeleteMenu(menu) {
